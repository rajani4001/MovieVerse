...
const HindiMovies = () => {
  ...
  const [data, setData] = useState({ featured: [], popular: [], nowPlaying: [], upcoming: [], topRated: [] });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [hindiRes, featuredRes] = await Promise.allSettled([
          api.get('/movies/hindi'),
          api.get('/movies/hindi-featured'),
        ]);
        const hindi    = hindiRes.status === 'fulfilled' ? hindiRes.value.data : {};
        const featured = featuredRes.status === 'fulfilled' ? featuredRes.value.data : [];
        const merged   = { ...hindi, featured };
        setData(merged);
        setInfiniteMovies(merged.popular || []);
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  // Hero = featured[0] — unique to Hindi page
  const heroMovie = data.featured?.[0] || data.popular?.[0];
  ...
  return (
    <div className="pb-12 bg-[#141414]">
      ...
      <div className="px-3 sm:px-6 md:px-10 lg:px-16 mt-6 sm:mt-8">
        <Section title="🎬 Must Watch — Bollywood" badge="FEATURED" badgeColor="bg-[#E50914] text-white" movies={data.featured} cardProps={cardProps} />
        <Section title="Popular Hindi Movies" badge="POPULAR" badgeColor="bg-orange-600 text-white" movies={data.popular} cardProps={cardProps} />
        <Section title="Ab Cinemas Mein" badge="NOW PLAYING" badgeColor="bg-green-600 text-white" movies={data.nowPlaying} cardProps={cardProps} />
        <Section title="Jald Aane Wali" badge="UPCOMING" badgeColor="bg-yellow-500 text-black" movies={data.upcoming} cardProps={cardProps} />
        <Section title="Sabse Behtareen Hindi Movies" badge="TOP RATED" badgeColor="bg-purple-600 text-white" movies={data.topRated} cardProps={cardProps} />

        <div className="mb-4 px-1">
          <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-white">Aur Filmen Dekho</h2>
        </div>
        <InfiniteScroll
          ...
          hasMore={hasMore}
          loader={<Loader />}
          endMessage={<p className="text-center text-gray-500 py-8 text-sm">Bas itni hi filmen hain!</p>}
